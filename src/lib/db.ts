...
/**
 * 🔴 **ענף רדום — והתנאי להתעוררותו מוצהר כאן במפורש.**
 *
 * הפונקציה נכונה, אבל היא שואלת על `accounts.external_ref` — ועמודה זו
 * **אין לה היום אף אתר כתיבה בכל הריפו**. לכן היא מחזירה `null` תמיד,
 * ו-`import_batches.account_id` נשאר `null`. זה אינו כשל: המיגרציה מצהירה
 * ש-`null` פירושו «לא זוהה», ואף לוגיקה אינה תלויה בערך.
 *
 * ⚠️ **התנאי להפעלה:** ברגע שייווצר מסלול שכותב ל-`accounts.external_ref`
 *    — למשל יצירת חשבון מתוך ייבוא דוח עו"ש — הענף יתחיל לירות מעצמו,
 *    בלי ששורה כאן תשתנה. זו הסיבה שהוא מתועד ולא נמחק.
 *
 * 🎯 ולכן השער אינו הערה: `d20.mjs` §K סופר את אתרי הכתיבה ו**נופל**
 *    כשמופיע הראשון. מי שמוסיף כתיבה נדרש להחליט במודע אם הענף מתעורר,
 *    ואינו יכול להעיר אותו בשקט. הערה לבדה מתיישנת ואיש אינו שם לב —
 *    זה כבר קרה כאן שלוש פעמים בבדיקות שמדדו הערה במקום התנהגות.
 */
