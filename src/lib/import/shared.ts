...
/**
 * המזהה משורת חיוב מרוכז בדוח העו"ש: «4003 - כרטיסי אשראי לי» → `4003`.
 *
 * 🎯 זו החוליה שסוגרת את השרשרת: דוח האשראי מצהיר «כרטיס:4003», ושורת
 *    העו"ש מתחילה באותו מספר. שני קבצים נפרדים, אותו מזהה.
 *
 * 🔴 **המזהה נלקח כשלמותו, כולל מקפים פנימיים.** הגרסה הקודמת הייתה
 *    `/^(\d{3,})\s*-/` והיא **קטעה בשקט**: מדדתי `«363-313550 - חיוב»`
 *    ⇒ `'363'`. זה לא `null` אלא **מזהה שגוי ולא-ריק**, כלומר בדיוק
 *    הכשל השקט שאנחנו מחסלים לאורך כל הסבב. מזהי חשבון אמיתיים נמדדו
 *    בשלושת הקבצים בצורה `363-313550` ו-`12-556-568338`.
 *
 * ⚠️ ו-`12-556-568338` נכשל גם בשער ה-`\d{3,}` — הוא פותח בשתי ספרות
 *    בלבד. לכן הרצפה נמדדת על **סך הספרות** במזהה ולא על המקטע הראשון.
 *
 * ⚠️ הפונקציה אינה מתנה דבר ב-`ref_kind`. אצוות אשראי מזוהה ב-`account`
 *    (הפועלים) ואצוות אחרת ב-`card` (אוצר החייל) — התניה על הסוג הייתה
 *    יוצרת ענף מת בדיוק כמו `institution`. ההתאמה היא על הערך בלבד,
 *    ובצירוף הסכום.
 */
export function cardRefFromCharge(description: string): string | null {
  const m = norm(description).match(/^(\d+(?:-\d+)*)\s*-\s/);
  if (!m) return null;
  // הרצפה על סך הספרות: «4003» = 4, «12-556-568338» = 11, «5 - ...» = 1 ונדחה.
  return m[1].replace(/-/g, '').length >= MIN_CHARGE_REF_DIGITS ? m[1] : null;
}

/** פחות מכך אינו מזהה אלא מספר סידורי או מונה. */
const MIN_CHARGE_REF_DIGITS = 3;